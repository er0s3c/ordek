...
// İnsan-dostu kod: yalnız A-Z0-9, 3-12 hane.
const sanitizeCode = (c) => String(c || "").toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 12);
// Sınıf adından okunaklı bir kod türet (Türkçe harfleri sadeleştir), benzersizleştir; ör. GRUP1, 11ASIBER1.
function autoFriendlyCode(name, classes) {
  const TR = { "Ç": "C", "Ğ": "G", "İ": "I", "Ö": "O", "Ş": "S", "Ü": "U" };
  let base = String(name || "").toUpperCase().replace(/[ÇĞİÖŞÜ]/g, (m) => TR[m] || m).replace(/[^A-Z0-9]/g, "").slice(0, 8);
  if (base.length < 2) base = "GRUP";
  for (let n = 1; n < 1000; n++) { const c = base + n; if (!classes.some((x) => x.code === c)) return c; }
  let code; do { code = genCode(6); } while (classes.some((c) => c.code === code));
  return code;
}

export function createClass({ name, teacherId, code }) {
  const classes = loadClasses();
  let finalCode = sanitizeCode(code);
  if (finalCode.length >= 3) {
    if (classes.some((c) => c.code === finalCode)) throw new Error("Bu sınıf kodu zaten kullanımda — başka bir kod seç.");
  } else {
    finalCode = autoFriendlyCode(name, classes);
  }
  const cls = {
    id: genId("c"),
...
export function updateClass(id, patch) {
  ...
  if (patch.code) {
    const c = sanitizeCode(patch.code);
    if (c.length >= 3) {
      if (classes.some((x) => x.code === c && x.id !== id)) throw new Error("Bu sınıf kodu zaten kullanımda.");
      classes[i].code = c;
    }
  }
