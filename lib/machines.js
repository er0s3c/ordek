...
export const MACHINES = {
  ...
  "nmap-recon": {
    name: "Nmap — Servis Keşfi",
    endpoint: { method: "GET", path: "/nmap-recon", kind: "query", sample: "ports=all" },
    story: "Hedef sunucu birden çok servis çalıştırıyor. Standart bir tarama yalnız yaygın portları gösterir; ama yüksek bir portta gizli bir servis var. Nmap mantığıyla tüm portları (-p-) tarayıp gizli servisin banner'ındaki flag'i bul.",
    objectives: ["Varsayılan taramayı çalıştır, açık servisleri listele", "`-p-` (tüm portlar) ile gizli yüksek portu (31337) keşfet", "Gizli servisin banner'ından flag'i oku", "High'da servisin firewall ile filtrelendiğini gözle"],
  },
  "metasploit-rce": {
    name: "Metasploit — Servis Exploit (RCE)",
    endpoint: { method: "POST", path: "/metasploit-rce", kind: "json", sample: '{"cmd":"id","token":""}' },
    story: "Hedefte bilinen zafiyetli bir yönetim servisi (OrdekAdmin 1.0) çalışıyor. Metasploit modülü mantığıyla servise komut çalıştır (RCE) ve meterpreter oturumu açarak flag'i ele geçir. Banner'da sızan varsayılan yönetim token'ına dikkat et.",
    objectives: ["GET ile servis banner'ını + sürümünü incele (bilgi ifşası)", "`exploit` modülünü çalıştır: cmd gönder → RCE", "Medium'da sızan varsayılan token'ı (set TOKEN) kullanarak zayıf yetki kontrolünü atlat", "High'da hedefin yamalandığını (not vulnerable) gör"],
  },

  /* ───────────────────────── FAZ 2 — 10 YENİ ZAFİYET ───────────────────────── */
  "xxe": {
    name: "XXE (XML External Entity)",
    endpoint: { method: "POST", path: "/xxe", kind: "raw", sample: '<?xml ...><!DOCTYPE r [<!ENTITY x SYSTEM "file:///etc/passwd">]><r>&x;</r>' },
    story: "Fatura içe-aktarma servisi XML ayrıştırıyor ama harici varlıkları (external entity) çözmeyi kapatmamış. Bir XXE payload'u ile sunucudaki gizli dosyayı oku.",
    objectives: ["XML gövdesine bir DOCTYPE + ENTITY tanımı ekle", "`file://` ile sunucudaki gizli flag dosyasını oku", "Medium'da DOCTYPE filtresini bypass et, High'da entity'lerin kapalı olduğunu gör"],
  },
  "nosql-injection": {
    name: "NoSQL Injection",
    endpoint: { method: "POST", path: "/nosql", kind: "json", sample: '{"username":"admin","password":{"$ne":""}}' },
    story: "Giriş API'si gelen JSON'u doğrudan bir Mongo-tarzı sorguya koyuyor. Operatör enjeksiyonu (`$ne`, `$gt`, `$regex`) ile parola doğrulamasını atlayıp admin ol.",
    objectives: ["Parola alanına `{\"$ne\":\"\"}` operatörü gönder", "Auth bypass ile admin oturumu aç", "Medium'da operatör kara listesini `$regex` ile atlat"],
  },
  "graphql-injection": {
    name: "GraphQL Introspection & Injection",
    endpoint: { method: "POST", path: "/graphql", kind: "json", sample: '{"query":"{ __schema { types { name } } }"}' },
    story: "GraphQL API'sinde introspection açık ve bir alan yetki kontrolü yapmıyor. Şemayı keşfet, gizli `secretNote` alanını sorgula ve flag'i çek.",
    objectives: ["Introspection sorgusuyla şemayı dök", "Gizli (yetkisiz) alanı keşfet", "O alanı sorgulayıp flag'i al; High'da introspection kapalı"],
  },
  "ldap-injection": {
    name: "LDAP Injection",
    endpoint: { method: "POST", path: "/ldap", kind: "form", sample: "username=*)(uid=*))(|(uid=*&password=x" },
    story: "Kurumsal giriş LDAP filtresine kullanıcı girdisini birleştiriyor. Filtre enjeksiyonu (`*`, `)(`) ile kimlik doğrulamayı atlat.",
    objectives: ["Kullanıcı adına LDAP meta karakterleri (`*`, `)(`) gir", "Filtre mantığını her-zaman-doğru yap → auth bypass", "Medium'da karakter filtresini atlat, High'da kaçışlandığını gör"],
  },
  "xpath-injection": {
    name: "XPath Injection",
    endpoint: { method: "POST", path: "/xpath", kind: "form", sample: "username=' or '1'='1&password=x" },
    story: "Kullanıcılar bir XML dosyasında tutuluyor ve giriş XPath sorgusuyla doğrulanıyor. XPath enjeksiyonu ile tüm kullanıcıları dök veya admin olarak gir.",
    objectives: ["XPath sorgusunu `' or '1'='1` ile manipüle et", "Auth bypass / kullanıcı dökümü yap", "High'da parametrelendiğini gözle"],
  },
  "http-parameter-pollution": {
    name: "HTTP Parameter Pollution (HPP)",
    endpoint: { method: "GET", path: "/hpp", kind: "query", sample: "role=user&role=admin&amount=100" },
    story: "Para transferi onayı aynı parametreyi birden çok katmanda farklı okuyor. Parametreyi çift göndererek (HPP) yetki/iş-mantığı kontrolünü atlat.",
    objectives: ["Aynı parametreyi iki kez gönder (ilk vs son okuma farkı)", "Yetki/limit kontrolünü çakıştır → bypass", "High'da tutarlı ayrıştırma ile engellendiğini gör"],
  },
  "web-cache-poisoning": {
    name: "Web Cache Poisoning",
    endpoint: { method: "GET", path: "/cache-poison", kind: "query", sample: "X-Forwarded-Host: evil.com (header)" },
    story: "Ana sayfa, önbelleğe alınırken anahtarsız (unkeyed) bir başlığı yansıtıyor. Zehirli bir başlıkla önbelleği bozup sonraki ziyaretçilere XSS/zararlı içerik servis et.",
    objectives: ["`X-Forwarded-Host` gibi anahtarsız başlığı yansıt", "Önbelleği zehirle (sonraki istek aynı zehirli yanıtı alır)", "High'da başlığın anahtarlandığını/temizlendiğini gör"],
  },
  "websocket-tampering": {
    name: "WebSocket Message Tampering",
    endpoint: { method: "POST", path: "/ws-chat", kind: "json", sample: '{"action":"send","role":"admin","text":"..."}' },
    story: "Canlı destek 'WebSocket' mesajlarında istemcinin gönderdiği `role` alanına güveniyor. Mesaj gövdesini kurcalayarak admin komutları çalıştır.",
    objectives: ["Mesaj JSON'una `role:admin` ekleyerek yetki yükselt", "Admin-only `getSecret` aksiyonunu çağır", "High'da rolün sunucu oturumundan alındığını gör"],
  },
  "git-disclosure": {
    name: ".git / Yedek Dosya İfşası",
    endpoint: { method: "GET", path: "/git-disclosure", kind: "query", sample: "path=/.git/config" },
    story: "Dağıtım sırasında `.git` klasörü ve `config.php.bak` yedeği siteye sızmış. Sürüm kontrol/yedek dosyalarından kaynak kodu ve gömülü sırrı çek.",
    objectives: ["`/.git/` ve yedek (`.bak`) yollarını keşfet", "Kaynak/yapılandırmadaki gömülü sırrı oku", "High'da bu yolların engellendiğini gör"],
  },
  "jwt-alg-confusion": {
    name: "JWT Algorithm Confusion (RS256→HS256)",
    endpoint: { method: "POST", path: "/jwt-confusion", kind: "json", sample: '{"token":"<HS256, public key ile imzalı>"}' },
    story: "API, RS256 ile imzalı JWT bekliyor ama doğrulayıcı `alg` başlığına körü körüne güveniyor. Public anahtarı HMAC sırrı gibi kullanıp (HS256) admin token'ı forge et.",
    objectives: ["Public anahtarın sızdığını fark et", "Token'ı HS256 + public anahtarla yeniden imzala (alg confusion)", "role:admin ile flag'i al; High'da alg sabitlendiği için reddedilir"],
  },
};
